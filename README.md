...
1.	方法区: 存储已被虚拟机加载的类信息、静态变量、即时编译后代码等数据。并使用永久代来实现方法区，1.8后被元空间替代，元空间并不在虚拟机中，而是使用本地内存。
...
初始化时机：new、静态字段或方法被使用、反射、父类、main
...
当Eden区空间不足以继续分配对象，发起新生代GC。

####老年代：

#####进入条件

1. 大对象（避免新生代GC以及分配担保机制和Eden与Survivor之间的复制）进入老年代  
...
4. 如果Survivor空间无法容纳新生代中Minor GC之后还存活的对象，通过分配担保机制进入老年代
  
#####Full GC条件

6 update24之前：老年代的连续空间小于新生代对象总大小并且HandlePromotionFailure不允许担保失败或者老年代的连续空间小于新生代对象总大小并且HandlePromotionFailure允许担保失败并且连续空间小于历次晋升到老年代的对象的平均大小  
6 update24之后：老年代的连续空间小于新生代对象总大小和历次晋升到老年代的对象的平均大小
